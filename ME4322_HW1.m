...
eqn10=cross(posGF,-FF)+cross(posGWeight,Force)+cross(posw5,w5)==0;
...
eqn2=cross(omegaDE,posDE)+cross(omegaEF,posEF)+cross(omegaFG,posGF)==0;
...
v_BC=2*double(solution.wBC)/BC
v_DE=2*double(solution.wDE)/DE
v_EF=2*double(solution.wEF)/EF
v_FG=2*double(solution.wFG)/GF
...
eqn2=cross(alphaEF,posEF)+cross(omegaEF, cross(omegaEF,posEF))+cross(alphaFG,posGF)+cross(omegaFG, cross(omegaFG,posGF))+cross(alphaDE,posDE)+cross(omegaDE, cross(omegaDE,posDE))==0;
...
la_BC=2*double(solution.aBC)/BC
la_DE=2*double(solution.aDE)/DE
la_EF=2*double(solution.aEF)/EF
la_FG=2*double(solution.aFG)/GF

%dynamic force/torque calcs
F_BC=massBC*2*double(solution.aBC)/BC
F_DE=massDE*2*double(solution.aDE)/DE
F_EF=massEF*2*double(solution.aEF)/EF
F_FG=massFG*2*double(solution.aFG)/GF
T_in=cross(posAB,[0 0 0])
...
new_laBC=zeros(360,1);
new_laDE=zeros(360,1);
new_laEF=zeros(360,1);
new_laFG=zeros(360,1);
...
for theta=0:1:359
    ...
    %linear velocity calcs
    new_vBC(theta+1)=2*double(solution.wBC)/BC;
    new_vDE(theta+1)=2*double(solution.wDE)/DE;
    new_vEF(theta+1)=2*double(solution.wEF)/EF;
    new_vFG(theta+1)=2*double(solution.wFG)/GF;
    ...
    %linear acceleration calcs
    new_laBC(theta+1)=2*double(solution.aBC)/BC;
    new_laDE(theta+1)=2*double(solution.aDE)/DE;
    new_laEF(theta+1)=2*double(solution.aEF)/EF;
    new_laFG(theta+1)=2*double(solution.aFG)/GF;

    %dynamic force calcs
    new_FBC(theta+1)=massBC*2*double(solution.aBC)/BC;
    new_FDE(theta+1)=massDE*2*double(solution.aDE)/DE;
    new_FEF(theta+1)=massEF*2*double(solution.aEF)/EF;
    new_FFG(theta+1)=massFG*2*double(solution.aFG)/GF;
    dynTorque=cross(posAB,[0 0 0]);
    new_Tin(theta+1)=dynTorque(3);
...
figure
    ...
    ax18=  subplot(2,2,1);
    plot(1:360,new_vBC);
    title(ax18,'Linear Velocity of Link BC')
    ax19=  subplot(2,2,2);
    plot(1:360,new_vDE);
    title(ax19,'Linear Velocity of Link DE')
    ax20=  subplot(2,2,3);
    plot(1:360,new_vEF);
    title(ax20,'Linear Velocity of Link EF')
    ax21=  subplot(2,2,4);
    plot(1:360,new_vFG);
    title(ax21,'Linear Velocity of Link FG')
...
figure
    ...
    ax26=  subplot(2,2,1);
    plot(1:360,new_laBC);
    title(ax26,'Linear Acceleration of Link BC')
    ax27=  subplot(2,2,2);
    plot(1:360,new_laDE);
    title(ax27,'Linear Acceleration of Link DE')
    ax28=  subplot(2,2,3);
    plot(1:360,new_laEF);
    title(ax28,'Linear Acceleration of Link EF')
    ax29=  subplot(2,2,4);
    plot(1:360,new_laFG);
    title(ax29,'Linear Acceleration of Link FG')

figure

    ax30=  subplot(2,3,1);
    plot(1:360,new_FBC);
    title(ax30,'Dynamic Force @ Link BC')
    ax31=  subplot(2,3,2);
    plot(1:360,new_FDE);
    title(ax31,'Dynamic Force @ Link DE')
    ax32=  subplot(2,3,3);
    plot(1:360,new_FEF);
    title(ax32,'Dynamic Force @ Link EF')
    ax33=  subplot(2,3,4);
    plot(1:360,new_FFG);
    title(ax33,'Dynamic Force @ Link FG')
    ax34=  subplot(2,3,5);
    plot(1:360,new_Tin);
    title(ax34,'Dynamic Input Torque')
