...
posEF=F-E;
posFG=G-F;
posWeight=F-posForce;

%masses of links (kg) (from solidworks)
massAB=23.34;
massBC=56.43;
massDE=98.46;
massEF=47;
...
%mass moments of inertia of links (kg*m^2) (from solidworks)
%format: [Ixx Iyy Izz]
inertiaAB=7438.54;
inertiaBC=98547.7;
inertiaDE=518704.8;
...
%Eqn for Link DE
eqn5=-FC+FD+FE+w3==0;
eqn6=cross(posCE,FE)+cross(posCD,FD)+cross(posw3,w3)==0;
...
eqn10=cross(posFG,FG)+cross(-posWeight,Force)+cross(posw5,w5)==0;
...
omegaEF=[0 0 wEF];
omegaFG=[0 0 wFG];
eqn1=cross(wAB,posAB)+cross(omegaBC,posBC)+cross(omegaDE,posCD)==0;
...
v_EA=cross([0 0 w_DE], posCE)+v_CA
...
eqn1=cross(aAB,posAB)+cross(wAB, cross(wAB,posAB))+cross(alphaBC,posBC)+cross(omegaBC, cross(omegaBC,posBC))+cross(alphaDE,posCD)+cross(omegaDE, cross(omegaDE,posCD))==0;
...
la_BA=cross(wAB, cross(wAB,posAB))
la_CA=cross([0 0 a_BC], posBC)+cross([0 0 w_BC], cross([0 0 w_BC], posBC))+la_BA
la_EA=cross([0 0 a_DE], posCE)+cross([0 0 w_DE], cross([0 0 w_DE], posCE))+la_CA
la_FA=cross([0 0 a_EF], posEF)+cross([0 0 w_EF], cross([0 0 w_EF], posEF))+la_EA

%CoM acceleration calcs
ac_AB=cross(wAB, cross(wAB,posw1));
ac_BC=cross([0 0 a_BC], posw2)+cross([0 0 w_BC], cross([0 0 w_BC], posw2))+la_BA;
ac_DE=cross([0 0 a_DE], posCE-posw3)+cross([0 0 w_DE], cross([0 0 w_DE], posCE-posw3))+la_CA;
ac_EF=cross([0 0 a_EF], posw4)+cross([0 0 w_EF], cross([0 0 w_EF], posw4))+la_EA;
ac_FG=cross([0 0 a_FG], posw5)+cross([0 0 w_FG], cross([0 0 w_FG], posw5))+la_FA;

%Eqn for Link AB
eqn1=FA+FB+w1==massAB*ac_AB;
eqn2=cross(posAB,FB)+cross(posw1,w1)+Torque==0;

%Eqn for Link BC
eqn3=-FB+FC+w2==massBC*ac_BC;
eqn4=cross(posBC,FC)+cross(posw2,w2)==inertiaBC*[0 0 a_BC];

%Eqn for Link DE
eqn5=-FC+FD+FE+w3==massDE*ac_DE;
eqn6=cross(posCE,FE)+cross(posCD,FD)+cross(posw3,w3)==inertiaDE*[0 0 a_DE];

%Eqn for Link EF
eqn7=-FE+FF+w4==massEF*ac_EF;
eqn8=cross(posEF,FF)+cross(posw4,w4)==inertiaEF*[0 0 a_EF];

%Eqn for Link FG
eqn9=-FF+FG+Force+w5==massFG*ac_FG;
eqn10=cross(posFG,FG)+cross(-posWeight,Force)+cross(posw5,w5)==inertiaFG*[0 0 a_FG];
...
for theta=0:1:359
    ...
    %linear acceleration calcs
    laBA=cross(wAB, cross(wAB,posAB));
    new_laBA(theta+1)=norm(laBA);
    laCA=cross([0 0 double(solution.aBC)], posBC)+cross(omegaBC, cross(omegaBC, posBC))+laBA;
    new_laCA(theta+1)=norm(laCA);
    laEA=cross([0 0 double(solution.aDE)], posCE)+cross(omegaDE, cross(omegaDE, posCE))+laCA;
    new_laEA(theta+1)=norm(laEA);
    laFA=cross([0 0 double(solution.aEF)], posEF)+cross(omegaEF, cross(omegaEF, posEF))+laEA;
    new_laFA(theta+1)=norm(laFA);

    %CoM acceleration calcs
    ac_AB=cross(wAB, cross(wAB,posw1));
    ac_BC=cross([0 0 double(solution.aBC)], posw2)+cross(omegaBC, cross(omegaBC, posw2))+laBA;
    ac_DE=cross([0 0 double(solution.aDE)], posCE-posw3)+cross(omegaDE, cross(omegaDE, posCE-posw3))+laCA;
    ac_EF=cross([0 0 double(solution.aEF)], posw4)+cross(omegaEF, cross(omegaEF, posw4))+laEA;
    ac_FG=cross([0 0 double(solution.aFG)], posw5)+cross(omegaFG, cross(omegaFG, posw5))+laFA;
    ...
    %Eqn for Link BC
    eqn3=-FB+FC+w2==massBC*ac_BC;
    eqn4=cross(posBC,FC)+cross(posw2,w2)==inertiaBC*[0 0 double(solution.aBC)];
    
    %Eqn for Link DE
    eqn5=-FC+FD+FE+w3==massDE*ac_DE;
    eqn6=cross(posCE,FE)+cross(posCD,FD)+cross(posw3,w3)==inertiaDE*[0 0 double(solution.aDE)];
    
    %Eqn for Link EF
    eqn7=-FE+FF+w4==massEF*ac_EF;
    eqn8=cross(posEF,FF)+cross(posw4,w4)==inertiaEF*[0 0 double(solution.aEF)];
    
    %Eqn for Link FG
    eqn9=-FF+FG+Force+w5==massFG*ac_FG;
    eqn10=cross(posFG,FG)+cross(-posWeight,Force)+cross(posw5,w5)==inertiaFG*[0 0 double(solution.aFG)];
